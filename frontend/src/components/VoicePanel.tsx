...
export function VoicePanel({
  ...
  const isDenied =
    ...
    state.error?.toLowerCase().includes("denied") ||
    state.error?.toLowerCase().includes("permission") ||
    state.error?.toLowerCase().includes("blocked");

  const isInsecure = state.error?.toLowerCase().includes("https");
  ...
  return (
    <div className="flex flex-col items-end gap-2">
      ...
      {showGuide && (
        <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="w-full max-w-sm bg-card border-2 border-amber-200/20 rounded-3xl p-6 shadow-2xl animate-in zoom-in-95 duration-200">
            <h3 className="font-display text-xl text-amber-200 mb-4 text-center">
              How to Allow Mic
            </h3>
            <div className="space-y-4 text-sm text-amber-100/80">
              {[
                <>
                  Look at the{" "}
                  <span className="text-amber-400 font-bold">address bar</span>{" "}
                  at the top of your browser.
                </>,
                <>
                  Click the{" "}
                  <span className="text-amber-400 font-bold">Lock icon</span> or{" "}
                  <span className="text-amber-400 font-bold">Mic icon</span> next
                  to the URL.
                </>,
                <>
                  Switch <span className="text-amber-400 font-bold">Microphone</span>{" "}
                  to{" "}
                  <span className="text-green-400 font-bold">Allow</span>.
                </>,
        ...
        >
          ...
          <div className="flex items-center justify-between px-3 py-2 border-b border-amber-200/10 flex-shrink-0">
            <div className="flex items-center gap-1.5">
              ...
              <span className="font-display text-xs text-amber-200/80 tracking-wide">
                VOICE CHAT
              </span>
              {state.enabled && (
            ...
            <div className="flex items-center gap-1">
              <button
                ...
                className="text-amber-200/40 hover:text-amber-200/80 transition-colors px-1 flex items-center"
              >
                <SettingsIcon size={15} color="#fbbf2466" />
              </button>
              <button
                onClick={() => setPanelOpen(false)}
                className="text-amber-200/40 hover:text-amber-200/80 text-lg leading-none transition-colors ml-1"
              >
                ×
              </button>
            </div>
            ...
            <div className="mx-2 mt-2 px-3 py-2 rounded-xl bg-red-500/15 border border-red-400/20 text-[10px] text-red-300 font-display">
              {isDenied ? (
                <div className="flex flex-col gap-1.5">
                  <p>Mic blocked. Allow it in your browser settings.</p>
                  <button
                ...
                <div className="flex flex-col gap-1.5">
                  <p className="text-amber-300 font-bold">HTTPS required</p>
                  <p>{state.error}</p>
                </div>
              ) : (
                state.error
      ...
      {/* Toggle button — floating mic button */}
      <button
