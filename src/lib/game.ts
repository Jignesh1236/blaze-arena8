...
export function buildDeck(): Card[] {
  ...
  // 4 wild 8s and 4 Switcheroos (K), all suitless
  for (let i = 0; i < 4; i++) {
...
/** Can `card` be played on top of `topCard` given the active suit? */
export function canPlay(card: Card, topCard: Card, currentSuit: Suit, _drawCount: number): boolean {
  if (card.rank === "8" || card.rank === "K") return true; // wilds — playable anytime
